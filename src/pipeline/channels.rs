...
mod tests {
    ...
    use crate::classifier::Triage;
    use crate::nfs::{AuthCreds, ExportAccessOptions, NfsAttrs, NfsFh, NfsFileType, NfsVersion};
    use chrono::Utc;
    use tokio::sync::mpsc::error::TrySendError;

    #[tokio::test]
    async fn channels_created_with_specified_bounds() {
        ...
        let msg = ExportMsg {
            ...
            harvested_uids: vec![AuthCreds::root()],
            misconfigs: vec![],
        };
        channels.export_tx.send(msg).await.unwrap();
        let received = channels.export_rx.recv().await.unwrap();
        assert_eq!(received.host, "10.0.0.1");
        assert_eq!(received.export_path, "/data");
    }

    #[test]
    fn default_channel_bound_constants() {
        assert_eq!(DEFAULT_EXPORT_CHANNEL_BOUND, 5000);
        assert_eq!(DEFAULT_FILE_CHANNEL_BOUND, 50000);
        assert_eq!(DEFAULT_RESULT_CHANNEL_BOUND, 10000);
    }

    #[tokio::test]
    async fn export_msg_traverses_channel() {
        let mut channels = PipelineChannels::default();
        let creds = AuthCreds::root();
        let msg = ExportMsg {
            host: "192.168.1.10".into(),
            export_path: "/exports/share".into(),
            nfs_version: NfsVersion::V3,
            access_options: ExportAccessOptions::default(),
            harvested_uids: vec![creds],
            misconfigs: vec![],
        };
        channels.export_tx.send(msg).await.unwrap();
        let received = channels.export_rx.recv().await.unwrap();
        assert_eq!(received.host, "192.168.1.10");
        assert_eq!(received.export_path, "/exports/share");
        assert_eq!(received.nfs_version, NfsVersion::V3);
        assert_eq!(received.harvested_uids.len(), 1);
        assert_eq!(received.harvested_uids[0], AuthCreds::root());
        assert!(received.misconfigs.is_empty());
    }

    #[tokio::test]
    async fn file_msg_traverses_channel() {
        let mut channels = PipelineChannels::default();
        let attrs = NfsAttrs {
        ...
        let msg = FileMsg {
            ...
            harvested_uids: vec![AuthCreds::new(1000, 1000)],
        };
        channels.file_tx.send(msg).await.unwrap();
        let received = channels.file_rx.recv().await.unwrap();
        assert_eq!(received.host, "10.0.0.5");
        assert_eq!(received.export_path, "/data");
        assert_eq!(received.file_path, "secrets/id_rsa");
        assert_eq!(received.file_handle, NfsFh::default());
        assert!(received.attrs.is_file());
        assert_eq!(received.attrs.size, 4096);
        assert_eq!(received.attrs.mode, 0o644);
        assert!(matches!(received.reader, FileReader::Nfs { .. }));
        assert_eq!(received.harvested_uids.len(), 1);
        assert_eq!(received.harvested_uids[0], AuthCreds::new(1000, 1000));
    }

    #[tokio::test]
    async fn result_msg_traverses_channel() {
        let mut channels = PipelineChannels::default();
        let now = Utc::now();
        let msg = ResultMsg {
            timestamp: now,
            host: "10.0.0.1".into(),
            export_path: "/share".into(),
            file_path: "config/.env".into(),
            triage: Triage::Red,
            rule_name: "KeepEnvFile".into(),
            matched_pattern: ".env".into(),
            context: Some("DB_PASSWORD=hunter2".into()),
            file_size: 256,
            file_mode: 0o644,
            file_uid: 1000,
            file_gid: 1000,
        ...
        channels.result_tx.send(msg).await.unwrap();
        let received = channels.result_rx.recv().await.unwrap();
        assert_eq!(received.timestamp, now);
        assert_eq!(received.host, "10.0.0.1");
        assert_eq!(received.export_path, "/share");
        assert_eq!(received.file_path, "config/.env");
        assert_eq!(received.triage, Triage::Red);
        assert_eq!(received.rule_name, "KeepEnvFile");
        assert_eq!(received.matched_pattern, ".env");
        assert_eq!(received.context, Some("DB_PASSWORD=hunter2".into()));
        assert_eq!(received.file_size, 256);
        assert_eq!(received.file_mode, 0o644);
        assert_eq!(received.file_uid, 1000);
        assert_eq!(received.file_gid, 1000);
        assert_eq!(received.last_modified, now);
    }
